...
int do_attestation(sgx_enclave_id_t eid, config_t *config) {
  ...
  if (config->server == NULL) {
    ...
    try {
      msgio = new MsgIO(config->server,
                        (config->port == NULL) ? DEFAULT_PORT : config->port);
    } catch (...) {
      exit(1);
    }
  }
  ...
  if (OPT_ISSET(flags, OPT_PUBKEY)) {
    ...
    status = enclave_ra_init(eid, &sgxrv, config->pubkey, b_pse, &ra_ctx,
                             &pse_status);
  } else {
    if (debug)
  ...
  status = sgx_ra_proc_msg2(
      ra_ctx, eid, sgx_ra_proc_msg2_trusted, sgx_ra_get_msg3_trusted, msg2,
  ...
  if (enclaveTrusted == Trusted) {
    ...
    eprintf("Enclave Trust is TRUSTED and COMPLICATED. The client is out of "
            "date and\nmay not be trusted in the future depending on the "
            "service provider's  policy.\n");
  } else {
    // Not Trusted, but client may be able to take action to become
    // trusted.

    eprintf("Enclave Trust is NOT TRUSTED and COMPLICATED. The client is out "
            "of date.\n");
  }
  ...
  int retPibCmp = memcmp(&emptyPIB, (void *)(&msg4->platformInfoBlob),
                         sizeof(sgx_platform_info_t));
  ...
  if (retPibCmp == 0) {
    ...
    if (debug) {
      eprintf("+++ PIB: ");
      print_hexstring(stderr, &msg4->platformInfoBlob,
                      sizeof(sgx_platform_info_t));
      print_hexstring(fplog, &msg4->platformInfoBlob,
                      sizeof(sgx_platform_info_t));
      eprintf("\n");
    }
    ...
    sgx_status_t ret = sgx_report_attestation_status(
        &msg4->platformInfoBlob, enclaveTrusted, &update_info);
    ...
    if (ret == SGX_ERROR_UPDATE_NEEDED) {
      ...
      if (update_info.pswUpdate) {
        eprintf("  * Intel SGX Platform Software needs to be updated to the "
                "latest version.\n");
      }

      if (update_info.csmeFwUpdate) {
        eprintf("  * The Intel Management Engine Firmware Needs to be Updated. "
                " Contact your\n");
        eprintf("    OEM for a BIOS Update.\n");
      }

      if (update_info.ucodeUpdate) {
        eprintf("  * The CPU Microcode needs to be updated.  Contact your OEM "
                "for a platform\n");
        eprintf("    BIOS Update.\n");
      }
      eprintf("\n");
  ...
  if (enclaveTrusted == Trusted || NotTrusted_ItsComplicated) {
    ...
    if (debug)
      eprintf("+++ ECALL enclave_ra_get_key_hash (MK) ret= 0x%04x\n", status);
